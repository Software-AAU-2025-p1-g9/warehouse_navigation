...
void shortest_path(node* start_node, node* goal_node, int size_x, int size_y, node** nodes,
                   map_data* map_datas, enum algorithm alg, edge*** path,int* len, edge* all_edges, int edge_count) {
    switch (alg) {
        case LPA_STAR: {
            int map_id = node_pos(size_x, start_node->x, start_node->y);

            if (map_datas[map_id].last_variable_node == NULL) {
                initialize_lpa_star(nodes, size_x, size_y, start_node, goal_node, map_datas, all_edges, edge_count);
            }

            lpa_star(start_node, goal_node, map_datas, map_id, nodes, size_x, size_y, all_edges, edge_count); //Print version
            //lpa_star(start_node, goal_node, map_datas, map_id, all_edges, edge_count); //Fast version
        ...
        case D_STAR_LITE: {
            ...
            if (map_datas[map_id].last_variable_node == NULL) {
                initialize_d_star_lite(nodes, size_x, size_y, start_node, goal_node, map_datas, all_edges, edge_count);
            }
            d_star_lite(start_node, goal_node, map_datas, map_id, nodes, size_x, size_y, all_edges, edge_count); //Print version
            //d_star_lite(start_node, goal_node, map_datas, map_id, all_edges, edge_count); //Fast version
        ...
        default: {
            ...
            reset_g(nodes, size_x, size_y, map_id);
            astar(nodes, start_node->x, start_node->y, goal_node->x, goal_node->y, map_id, total_area);
            find_shortest_path(path, len, start_node, goal_node, map_id);
        }
    }
}
...
void generate_worker_route(worker* w, int size_y, int size_x, node** nodes, map_data* map_datas, enum algorithm alg,
    ...
    int len_AB;
    edge** path_AB = NULL;
    shortest_path(A, B, size_x, size_y, nodes, map_datas, alg, &path_AB, &len_AB, edges, edge_count);

    // Goes from B -> C by finding edges between nodes
    int len_BC;
    edge** path_BC = NULL;
    shortest_path(B, C, size_x, size_y, nodes, map_datas, alg, &path_BC, &len_BC, edges, edge_count);

    // Goes from C -> A by finding edges between nodes
    int len_CA;
    edge** path_CA = NULL;
    shortest_path(C, A, size_x, size_y, nodes, map_datas, alg, &path_CA, &len_CA, edges, edge_count);
